<input type="hidden" id="shared_parent_id" name="shared_parent_id" value="<?php echo $parent_id; ?>">
<table id="document_list_dt" class="datatable" style="min-width: 90%;">
    ...
    <tr>
    	<td align="left" valign="top" width="7%" colspan="6">
	    	...
	    	<?php if($parent) { ?>
	    		<a href="javascript:void(0);" onclick="javascript:shared_document_list(<?php echo $parent['parent_id'] ?>);">Up to <?php echo $parent['title']; ?></a>
	    	<?php } else { echo "Root"; } ?>
    	</td>
    </tr>
    <?php foreach($documents as $key=>$value): ?>
    	<tr>
    		...
    		<td align="left" valign="top" width="25%">
    			<?php if($value['file_type'] != FILE) { ?>
    				<a href="javascript:void(0);" onclick="javascript:shared_document_list(<?php echo $value['id']; ?>);"><?php echo $value['title']; ?></a>
