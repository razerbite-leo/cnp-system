...
<section class="data">
	<form id="edit_user_form" name="edit_user_form" method="post" action="<?php echo url("super/save_firm_user"); ?>" enctype="multipart/form-data">
	...
	<input type="hidden" id="user_id" name="user_id" value="<?php echo $user['id']; ?>">
		<div id="form">
			...
			<select id="firm_name" name="firm_name" class="select validate[required] edit_user_form">
				<option value="">Please Select One</option>
				<?php foreach($firm as $key=>$value): ?>
					<option value="<?php echo $value['id']; ?>" <?php echo ($value['id'] == $user['firm_id'] ? 'selected="selected"' : ''); ?>><?php echo $value['firm_name']; ?></option>
				<?php endforeach; ?>
			</select>			
			<section class="clear"></section>
			...
			<select id="account_type" name="account_type" class="select validate[required] edit_user_form">
				...
				<option <?php echo ($user['account_type'] == SUPER_ADMIN ? 'selected="selected"' : ''); ?> value="<?php echo SUPER_ADMIN; ?>">Super Administrator</option>
				<option <?php echo ($user['account_type'] == TECH_ADMIN ? 'selected="selected"' : ''); ?> value="<?php echo TECH_ADMIN; ?>">Tech Administrator</option>
				<option <?php echo ($user['account_type'] == FIRM_ADMIN ? 'selected="selected"' : ''); ?> value="<?php echo FIRM_ADMIN; ?>">Firm Administrator</option>
				<option <?php echo ($user['account_type'] == TECH_STAFF ? 'selected="selected"' : ''); ?> value="<?php echo TECH_STAFF; ?>">Tech Staff</option>
			</select>
			<section class="clear"></section>
			...
			<section id="form02">
				...
				<ul>
					<li>
						...
						<input type="file" id="user_display_picture_file" name="user_display_picture_file" class="textbox02 edit_user_form">
					</li>
			...
			<section id="form_contact">
				...
				<div id="other_contact_list_wrapper">
					<ul>
						<li>
							<select id="contact_type" name="contact_type" class="select edit_user_form" onchange="javascript:filter_extension_user();">
						...
						<li><input type="text" id="contact_type_value" name="contact" class="textbox02 edit_user_form"></li>
						<li><input type="text" id="contact_extension_user" name="contact_extension" class="textbox03 edit_user_form" style="display:none;" placeholder="Extension"></li>
			...
			<br/>
			<button id="big_save" class="big_save_inner" type="button" onclick="$('#edit_user_form').submit();">
				<ul>
